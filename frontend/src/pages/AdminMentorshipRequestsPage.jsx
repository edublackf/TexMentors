...
function AdminMentorshipRequestsPage() {
    ...
    const ALL_STATUSES = [ // Definir todos los estados posibles para el selector del admin
        'pendiente', 'aceptada_mentor', 'rechazada_mentor', 'rechazada_admin', 
    ...
    const handleUpdateRequest = async (e) => {
        ...
        console.log("Current Mentor ID (from request):", currentMentorId, typeof currentMentorId);
        console.log("New Selected Mentor ID (from dropdown):", newMentorId, typeof newMentorId);
...
console.log("Update Data Object:", updateData); // <-- MUY IMPORTANTE
        ...
        try {
            ...
            const response = await mentorshipRequestService.updateRequest(editingRequest._id, updateData);
            setActionMessage(response.message || 'Solicitud actualizada exitosamente.');
            closeEditModal();
    ...
    const handleDeleteRequest = async (requestId, requestTitle) => {
        if (window.confirm(`¿Estás seguro de que quieres eliminar la solicitud "${requestTitle}"?`)) {
            try {
                ...
                setActionMessage(response.message || 'Solicitud eliminada exitosamente.');
        ...
        <div>
            ...
            {editingRequest && (
                <div style={{ border: '2px solid blue', padding: '20px', margin: '20px 0' }}>
                    ...
                    <p><strong>Estudiante:</strong> {editingRequest.studentUser?.nombre} {editingRequest.studentUser?.apellido}</p>
                    <form onSubmit={handleUpdateRequest}>
                        ...
                        <div style={{ marginTop: '10px' }}>
                            ...
                            <textarea 
                                ...
                                rows="3" style={{ width: '90%'}}
                            />
                        </div>
                        <button type="submit" style={{ marginTop: '10px' }} disabled={loading}>Guardar Cambios</button>
                        <button type="button" onClick={closeEditModal} style={{ marginLeft: '10px' }} disabled={loading}>Cancelar Edición</button>
                    </form>
                </div>
            )}
                    ...
                    <tbody>
                        {requests.map((req) => (
                            <tr key={req._id} style={{ backgroundColor: req.isDeleted ? '#ffe0e0' : ''}}>
                                ...
                                <td>{req.mentorUser?.nombre || '-'} {req.mentorUser?.apellido || ''}</td>
                                <td>{req.helpType?.name}</td>
                                <td>{req.status}</td>
                                <td>{new Date(req.createdAt).toLocaleDateString()}</td>
                                <td>
                                    <button onClick={() => openEditModal(req)} disabled={req.isDeleted}>Editar/Asignar</button>
                                    <button 
                                        onClick={() => handleDeleteRequest(req._id, req.title)}
                                        disabled={req.isDeleted}
                                        style={{ marginLeft: '5px', backgroundColor: req.isDeleted ? 'grey' : 'tomato', color: 'white'}}
                                    >
